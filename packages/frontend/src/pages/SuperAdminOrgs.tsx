...
export function SuperAdminOrgs() {
  ...
  const logoutOrganization = async (e: React.MouseEvent, org: any) => {
    e.stopPropagation()
    if (!window.confirm(`Log out every user in ${org.name} from all devices?`)) return
    try {
      const result = await api.superAdminLogoutCompany(org.id)
      addToast({ title: 'Organization sessions ended', description: result.message, variant: 'success' })
    } catch (error: any) {
      addToast({ title: 'Could not end organization sessions', description: error.message, variant: 'destructive' })
    }
  }
    ...
    <div className="space-y-6 max-w-7xl">
          ...
          {filtered.map(org => (
            <div key={org.id} className="group rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
              ...
              <div className="p-5">
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <button onClick={() => openOrganization(org.id)} className="truncate text-left text-lg font-bold text-sky-700 hover:underline dark:text-sky-300">{org.name}</button>
                ...
                <button onClick={(e) => logoutOrganization(e, org)} className="mb-3 flex w-full items-center justify-center gap-2 rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 transition-colors hover:bg-red-50 dark:border-red-900 dark:hover:bg-red-950/30"><LogOut size={14} /> Logout all organization users</button>
      ...
      {selectedOrg && <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/50 p-3" onMouseDown={event => { if (event.target === event.currentTarget) closeOrganization() }}>
        <section className="max-h-[90dvh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-white shadow-2xl dark:bg-slate-900">
          <div className="flex items-start gap-4 border-b p-5 sm:p-6"><OrgLogo org={selectedOrg} size="lg"/><div className="min-w-0 flex-1"><h2 className="text-xl font-bold text-slate-900 dark:text-white">{selectedOrg.name}</h2><p className="mt-1 text-sm text-muted-foreground">Registered organisation details</p></div><button onClick={closeOrganization} className="rounded-xl p-2 hover:bg-muted"><X size={20}/></button></div>
          <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-6">
            {[
              ['Email', selectedOrg.email, Mail], ['Phone', selectedOrg.phone, Phone], ['Website', selectedOrg.website, Globe],
              ['City', selectedOrg.addressCity, MapPin], ['Country', selectedOrg.addressCountry, MapPin],
              ['Language', langLabel(selectedOrg.language), Globe], ['Timezone', tzLabel(selectedOrg.timezone), Clock],
              ['Date format', dfLabel(selectedOrg.dateFormat), Clock], ['Users', selectedOrg._count?.users ?? selectedOrg.users?.length ?? 0, Users],
