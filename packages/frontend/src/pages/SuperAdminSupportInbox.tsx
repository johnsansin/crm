...
const filters: Record<string, string[]> = {
  All: [], Waiting: ['WAITING_FOR_AGENT'], Active: ['AGENT_ASSIGNED', 'AGENT_ACTIVE'], Resolved: ['RESOLVED', 'CLOSED'],
}
...
export function SuperAdminSupportInbox() {
  ...
  const loadConversation = useCallback(async (id: string) => {
    ...
    try {
      ...
      setSelected(detail.data); setMessages(messageResponse.data || [])
  ...
  useEffect(() => { const id = new URLSearchParams(location.search).get('id'); if (id) loadConversation(id).catch(() => {}) }, [loadConversation])
  useEffect(() => {
    bottomRef.current?.scrollIntoView({ behavior: 'smooth' })
  }, [messages, customerTyping])

  const onSocketEvent = useCallback((event: any) => {
    if (event.event === 'customer.typing') { setCustomerTyping(!!event.typing); if (event.typing) window.setTimeout(() => setCustomerTyping(false), 2500); return }
    loadList().catch(() => {})
  ...
  return <div className="flex min-h-[calc(100dvh-7rem)] flex-col gap-3 p-0 sm:gap-4 lg:h-[calc(100dvh-9rem)] lg:min-h-[620px]">
    ...
    <div className="grid min-h-[540px] flex-1 overflow-hidden rounded-xl border bg-card shadow-sm sm:rounded-2xl md:grid-cols-[280px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(360px,1fr)_260px]">
      <aside className={`${selected ? 'hidden md:flex' : 'flex'} min-h-0 min-w-0 flex-col border-r`}>
        ...
        <div className="flex-1 overflow-y-auto p-2">{loading ? [1,2,3,4].map(i => <div key={i} className="mb-2 h-24 animate-pulse rounded-xl bg-muted"/>) : visible.length === 0 ? <div className="grid h-full place-items-center p-8 text-center"><div><CheckCircle2 className="mx-auto mb-2 text-emerald-500"/><p className="font-medium">No conversations found</p><p className="text-xs text-muted-foreground">You’re all caught up.</p></div></div> : visible.map(item => <button key={item.id} onClick={() => loadConversation(item.id)} className={`mb-1 w-full rounded-xl border p-3 text-left transition ${selected?.id === item.id ? 'border-indigo-400 bg-indigo-50 dark:bg-indigo-950/30' : 'border-transparent hover:bg-muted'}`}><div className="flex items-start justify-between gap-2"><p className="truncate text-sm font-semibold">{item.company?.name || 'Organisation'}</p><span className={`shrink-0 rounded-full px-1.5 py-0.5 text-[9px] font-bold ${priorityClass[item.priority]}`}>{item.priority}</span></div><p className="mt-1 truncate text-xs font-medium">{item.subject}</p><p className="mt-1 truncate text-xs text-muted-foreground">{item.messages?.[0]?.content}</p><div className="mt-2 flex items-center justify-between text-[10px] text-muted-foreground"><span>{item.status.replaceAll('_', ' ')}</span><span>{new Date(item.lastMessageAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span></div></button>)}</div>
      </aside>
        ...
        {!selected ? <div className="m-auto text-center"><Headphones className="mx-auto mb-3 text-indigo-500" size={42}/><h2 className="font-semibold">Select a conversation</h2><p className="text-sm text-muted-foreground">Open a request to see its complete AI and human history.</p></div> : <>
          <header className="border-b p-2.5 sm:p-3">
            ...
            <div className="mt-2 flex w-full gap-2 overflow-x-auto pb-0.5">
              ...
              {selected.status === 'AGENT_ASSIGNED' && selected.assignedAgentId === user?.id && <button disabled={busy} onClick={() => action(() => api.adminSupportAccept(selected.id))} className="h-9 shrink-0 rounded-lg bg-indigo-600 px-3 text-xs font-semibold text-white">Accept assigned chat</button>}
          ...
          <footer className="border-t p-3">{error && <p className="mb-2 text-xs text-red-600">{error}</p>}{selected.status === 'AGENT_ACTIVE' ? <div className="flex gap-2"><textarea rows={1} value={text} onChange={e => { setText(e.target.value); sendTyping(true, true) }} onBlur={() => sendTyping(false, true)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send() } }} placeholder="Type a reply…" className="min-h-11 flex-1 resize-none rounded-xl border bg-muted/30 px-3 py-3 text-sm outline-none focus:border-indigo-400"/><button onClick={send} disabled={!text.trim() || busy} className="grid h-11 w-11 place-items-center rounded-xl bg-indigo-600 text-white disabled:opacity-50"><Send size={18}/></button></div> : <p className="text-center text-xs text-muted-foreground">{selected.status === 'AGENT_ASSIGNED' ? 'The assigned agent must accept this chat before replying.' : 'Accept this conversation to reply.'}</p>}</footer>
        </>}
      </main>
