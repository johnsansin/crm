...
export function SystemBackupSettings() {
  ...
  const run = useMutation({
    ...
    onSuccess: data => { refresh(); addToast({ title: 'Database backup completed', description: data.backup.emailError ? `Backup saved, but email failed: ${data.backup.emailError}` : data.backup.emailDelivered ? 'Backup created and emailed successfully.' : 'Backup created successfully in private storage.', variant: data.backup.emailError ? 'destructive' : 'success' }) },
    onError: (e: Error) => { refresh(); addToast({ title: 'Database backup failed', description: e.message, variant: 'destructive' }) },
  })
  ...
  return <div className="space-y-4">
    ...
    <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><CalendarClock size={18} /> Backup schedule</CardTitle></CardHeader><CardContent className="space-y-5">
      <div className="flex items-center justify-between gap-4 rounded-xl border bg-muted/20 p-4"><div><p className="text-sm font-medium">Automatic backups</p><p className="text-xs text-muted-foreground mt-0.5">Run scheduled full-system PostgreSQL backups.</p></div><Switch checked={form.enabled} onCheckedChange={v => set('enabled', v)} /></div>
    ...
    <Card><CardHeader><CardTitle className="text-base flex items-center gap-2"><HardDrive size={18} /> Backup history</CardTitle></CardHeader><CardContent>
      {!backups.length ? <div className="rounded-xl border border-dashed py-10 text-center text-sm text-muted-foreground">No database backups yet.</div> : <div className="space-y-2">{backups.map((backup: any) => <div key={backup.fileName} className="flex flex-col sm:flex-row sm:items-center gap-3 rounded-xl border bg-muted/10 p-3"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{backup.fileName}</p><p className="text-xs text-muted-foreground mt-0.5">{date(backup.modifiedAt)} · {size(backup.size)}</p></div><div className="grid grid-cols-2 sm:flex gap-2"><Button size="sm" variant="outline" onClick={() => api.downloadSystemBackup(backup.fileName)}><Download size={14} className="mr-1.5" />Download</Button><Button size="sm" variant="outline" disabled={!form.emailTo || email.isPending} onClick={() => email.mutate(backup.fileName)}><Mail size={14} className="mr-1.5" />Email</Button></div></div>)}</div>}
    </CardContent></Card>
  </div>
}
