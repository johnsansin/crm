...
import { brandSchema, canReview, postSchema, providers, tenantId } from './domain'

export const socialForceRouter = Router()
socialForceRouter.use(authMiddleware, requireTenant, requireModulePermission('social'))
...
socialForceRouter.get('/workspace', async (req, res, next) => {
  try {
    ...
    const [company, brand, posts, events] = await Promise.all([
      ...
      prisma.socialForceBrand.findUnique({ where: { companyId } }),
      prisma.socialForcePost.findMany({ where: { companyId }, orderBy: { updatedAt: 'desc' }, take: 200 }),
      prisma.socialForceEvent.findMany({ where: { companyId }, orderBy: { createdAt: 'desc' }, take: 30 }),
    ])
    res.json({ data: { company, brand, posts, events, providers, accounts: [], aiConfigured: false,
      canManage: !!(req.user!.isAdmin || req.user!.isSuperAdmin), userId: req.user!.userId,
      publishingEnabled: false, phase: 'FOUNDATION' } })
  } catch (err) { next(err) }
})
socialForceRouter.post('/posts', async (req, res, next) => {
  try {
...
socialForceRouter.put('/posts/:id', async (req, res, next) => {
  try {
...
socialForceRouter.post('/posts/:id/submit', async (req, res, next) => {
  try {
    const companyId = tenantId(req.user)
...
socialForceRouter.post('/posts/:id/review', requireAdmin, async (req, res, next) => {
  try {
    ...
    if (!canReview(req.user!, post.createdBy, req.user!.userId)) return res.status(403).json({ error: 'A different organization administrator must review this post.' })
    const count = await prisma.$transaction(async tx => {
...
socialForceRouter.delete('/posts/:id', async (req, res, next) => {
  try {
    const companyId = tenantId(req.user)
    const count = await prisma.$transaction(async tx => {
...
socialForceRouter.put('/brand', requireAdmin, async (req, res, next) => {
  try {
...
socialForceRouter.post('/providers/:provider/connect', requireAdmin, (_req, res) => res.status(503).json({ error: 'Official OAuth connection is not configured. An administrator must complete provider setup.' }))
socialForceRouter.post('/posts/:id/publish', (_req, res) => res.status(503).json({ error: 'Live publishing is not enabled. Save a draft while official integrations and publishing workers are being configured.' }))
socialForceRouter.post('/posts/:id/schedule', (_req, res) => res.status(503).json({ error: 'Automatic scheduling is not enabled. Planned dates are editorial reminders only.' }))
socialForceRouter.post('/ai/generate', (_req, res) => res.status(503).json({ error: 'AI generation is not configured. Use the manual editor or try sample content in the demo.' }))
