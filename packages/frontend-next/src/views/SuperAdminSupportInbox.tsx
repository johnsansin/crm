...
import { ArrowLeft, Building2, CheckCircle2, Headphones, Info, Loader2, Paperclip, Search, Send, UserRound, Wifi, WifiOff, X } from 'lucide-react'
...
const filters: Record<string, string[]> = {
  Waiting: ['WAITING_FOR_AGENT'], Active: ['AGENT_ACTIVE'], Assigned: ['AGENT_ASSIGNED'], Resolved: ['RESOLVED', 'CLOSED'], All: [],
}
...
export function SuperAdminSupportInbox() {
  ...
  async function sendFile(file?: File) {
    ...
    try {
      ...
      await api.adminSupportSend(selected.id, uploaded.fileName, createSupportMessageId(), uploaded)
      await Promise.all([loadList(), loadConversation(selected.id)])
  ...
  return <div className="relative isolate flex h-full min-h-0 w-full flex-1 flex-col gap-2 overflow-hidden rounded-xl border border-white/80 bg-gradient-to-br from-slate-50 via-indigo-50/50 to-violet-50/70 p-2 shadow-xl shadow-slate-900/5 dark:border-white/10 dark:from-slate-950 dark:via-indigo-950/20 dark:to-violet-950/20 sm:gap-2.5 sm:rounded-2xl sm:p-2.5 lg:p-3">
    ...
    <div className="relative z-20 flex shrink-0 flex-wrap items-center justify-between gap-2 rounded-2xl border border-white/80 bg-white/90 px-3.5 py-2.5 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-slate-900/90 sm:px-4 sm:py-3">
      <div className="shrink-0"><div className="mb-1 flex items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-indigo-600 to-violet-600 text-white shadow-lg shadow-indigo-500/20"><Headphones size={16}/></span><h1 className="whitespace-nowrap text-xl font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-2xl">Support Inbox</h1></div><p className="text-xs text-muted-foreground sm:text-sm">AI escalations and live organisation support, in one conversation.</p></div>
    ...
    <div className="relative z-0 grid min-h-0 flex-1 gap-2.5 sm:gap-3 md:grid-cols-[minmax(260px,300px)_minmax(0,1fr)] xl:grid-cols-[minmax(280px,328px)_minmax(360px,1fr)_minmax(250px,292px)]">
      <aside className={`flex min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl border border-white/80 bg-card/95 shadow-lg shadow-slate-900/5 backdrop-blur-xl dark:border-white/10 ${selected ? 'max-h-44 md:max-h-none' : ''}`}>
        <div className="shrink-0 border-b p-3 sm:p-3.5"><div className="relative"><Search className="absolute left-3 top-2.5 text-muted-foreground" size={15}/><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search organisation, user, message…" className="h-9 w-full rounded-lg border bg-muted/40 pl-9 pr-3 text-xs outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10"/></div><div className="mt-3 flex gap-1 overflow-x-auto">{Object.keys(filters).map(label => <button key={label} onClick={() => setActiveFilter(label)} className={`shrink-0 rounded-full px-2.5 py-1.5 text-[11px] font-semibold transition ${activeFilter === label ? 'bg-violet-600 text-white shadow-sm' : 'text-muted-foreground hover:bg-muted'}`}>{label} <span className="opacity-70">{filterCount(label)}</span></button>)}</div></div>
        <div className="flex-1 overflow-y-auto overscroll-contain">{loading ? <div className="p-2">{[1,2,3,4].map(i => <div key={i} className="mb-2 h-20 animate-pulse rounded-xl bg-muted"/>)}</div> : visible.length === 0 ? <div className="grid h-full place-items-center p-8 text-center"><div><CheckCircle2 className="mx-auto mb-2 text-emerald-500"/><p className="font-medium">No conversations found</p><p className="text-xs text-muted-foreground">You’re all caught up.</p></div></div> : visible.map(item => { const meta = statusMeta[item.status] || statusMeta.CLOSED; return <button key={item.id} onClick={() => loadConversation(item.id, true)} className={`relative w-full border-b p-3 text-left sm:p-3.5 transition ${selected?.id === item.id ? 'bg-violet-50 before:absolute before:inset-y-0 before:left-0 before:w-[3px] before:bg-violet-600 dark:bg-violet-950/30' : 'hover:bg-muted/50'}`}><div className="flex gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-violet-600 to-indigo-400 text-sm font-bold text-white">{(item.company?.name || 'O')[0].toUpperCase()}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className="truncate text-[13px] font-bold">{item.company?.name || 'Organisation'}</p><span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-bold ${priorityClass[item.priority]}`}>{item.priority === 'NORMAL' ? 'Normal' : item.priority}</span></div><p className="mt-0.5 truncate text-[11px] text-muted-foreground">{item.subject}</p><p className="mt-1.5 truncate text-xs text-muted-foreground">{item.messages?.[0]?.content || 'No messages yet'}</p><div className="mt-2 flex items-center justify-between gap-2"><span className={`flex items-center gap-1 text-[10px] font-semibold ${meta.text}`}><span className={`h-1.5 w-1.5 rounded-full ${meta.dot}`}/>{item.channel && item.channel !== 'SUPPORT' ? `${item.channel} · ` : ''}{item.status === 'AGENT_ASSIGNED' && item.assignedAgentId === user?.id ? 'Assigned to you' : meta.label}</span><span className="shrink-0 text-[10px] text-muted-foreground">{formatDateTime(item.lastMessageAt)}</span></div></div></div></button> })}</div>
      </aside>
        ...
        {!selected ? <div className="m-auto text-center"><Headphones className="mx-auto mb-3 text-indigo-500" size={42}/><h2 className="font-semibold">Select a conversation</h2><p className="text-sm text-muted-foreground">Open a request to see its complete AI and human history.</p></div> : <>
          <header className="border-b p-3.5 sm:p-4">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="min-w-0 flex-1"><h2 className="truncate text-sm font-extrabold sm:text-base">{selected.company?.name}</h2><p className="truncate text-[11px] text-muted-foreground sm:text-xs">{selected.channel || 'SUPPORT'} · {selected.subject} · {(statusMeta[selected.status] || statusMeta.CLOSED).label}</p></div>
            ...
            <div className="mt-2 flex w-full gap-2 overflow-x-auto pb-0.5">
              ...
              {selected.status === 'WAITING_FOR_AGENT' && <button disabled={busy} onClick={() => action(() => api.adminSupportClaim(selected.id))} className="h-9 shrink-0 rounded-lg bg-indigo-600 px-3 text-xs font-semibold text-white">{user?.isSuperAdmin ? 'Join conversation' : 'Accept conversation'}</button>}
              {selected.status === 'AGENT_ASSIGNED' && user?.isSuperAdmin && selected.assignedAgentId !== user.id && <button disabled={busy} onClick={() => action(() => api.adminSupportClaim(selected.id))} className="h-9 shrink-0 rounded-lg bg-violet-600 px-3 text-xs font-semibold text-white">Join conversation</button>}
