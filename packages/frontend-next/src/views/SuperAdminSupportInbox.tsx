...
export function SuperAdminSupportInbox() {
  ...
  return <div className="flex h-full min-h-0 flex-col gap-4 bg-slate-50/70 p-3 dark:bg-slate-950/20 sm:p-5">
    <div className="flex flex-wrap items-end justify-between gap-4">
      <div><h1 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Support Inbox</h1><p className="mt-1 text-sm text-muted-foreground">AI escalations and live organisation support, in one conversation.</p></div>
    ...
    <div className="grid min-h-0 flex-1 gap-3 md:grid-cols-[300px_minmax(0,1fr)] xl:grid-cols-[328px_minmax(380px,1fr)_292px]">
      <aside className={`${selected ? 'hidden md:flex' : 'flex'} min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl border bg-card shadow-sm`}>
      ...
      <main className={`${!selected ? 'hidden md:flex' : 'flex'} min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl border bg-card shadow-sm`}>
      ...
      <aside className="hidden overflow-y-auto rounded-2xl border bg-card p-5 shadow-sm xl:block">{selected && <CustomerContext selected={selected} isSuperAdmin={!!user?.isSuperAdmin} onOrganisation={() => navigate(`/superadmin/organizations?id=${selected.companyId}`)}/>}</aside>
    </div>
