...
export function ModuleListPage() {
  ...
  if (mod === 'leads') {
    ...
    const emailMeta = (status?: string) => {
      const key = status || 'NOT_CONTACTED'
      if (key === 'REPLIED') return { label: 'Replied', mark: 'bg-emerald-500', cls: 'bg-emerald-50 text-emerald-700' }
      if (key === 'EMAIL_DELIVERED') return { label: 'Delivered', mark: 'bg-emerald-500', cls: 'bg-emerald-50 text-emerald-700' }
      if (key === 'EMAIL_OPENED') return { label: 'Opened', mark: 'bg-teal-500', cls: 'bg-teal-50 text-teal-700' }
      if (key === 'EMAIL_CLICKED') return { label: 'Clicked', mark: 'bg-cyan-500', cls: 'bg-cyan-50 text-cyan-700' }
      if (key === 'AWAITING_REPLY') return { label: 'Awaiting Reply', mark: 'bg-amber-500', cls: 'bg-amber-50 text-amber-700' }
      if (key === 'BOUNCED' || key === 'DELIVERY_FAILED') return { label: key === 'BOUNCED' ? 'Bounced' : 'Delivery Failed', mark: 'bg-red-500', cls: 'bg-red-50 text-red-700' }
      if (key === 'SEQUENCE_ACTIVE' || key === 'EMAIL_SENT') return { label: key === 'SEQUENCE_ACTIVE' ? 'Sequence Active' : 'Email Sent', mark: 'bg-blue-500', cls: 'bg-blue-50 text-blue-700' }
      if (key === 'EMAIL_QUEUED') return { label: 'Queued', mark: 'bg-violet-500', cls: 'bg-violet-50 text-violet-700' }
      if (key === 'UNSUBSCRIBED') return { label: 'Unsubscribed', mark: 'bg-slate-400', cls: 'bg-slate-100 text-slate-600' }
      return { label: key === 'SEQUENCE_COMPLETED' ? 'Sequence Completed' : key === 'SEQUENCE_PAUSED' ? 'Sequence Paused' : 'Not Contacted', mark: 'bg-slate-300', cls: 'bg-slate-100 text-slate-600' }
    }
    const actionCards = [
