...
function applyUserLocale(user: any) {
  if (!user) return
  setOrgSettings({ language: user.language || 'en_us', timezone: user.timezone || 'UTC', dateFormat: user.dateFormat || 'mm-dd-yyyy', hourFormat: user.hourFormat || '12h' })
...
if (typeof window !== 'undefined') useAuthStore.getState().loadUser()
